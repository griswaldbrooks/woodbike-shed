...
page: P14
title: Trim
stage: Stage 11 of 12
prev: 13-skirt-and-siding.md
next: 15-doors.md
---

# Stage 11 — Trim

> **Goal:** every corner, eave line and door opening is trimmed — all siding edges covered and each opening framed for its door.
> **Crew:** 1 person · **Time:** about one day · **Weather:** dry

## Before you start

The skirt and all siding are on (Stage 10). Trim is the next layer out: **all trim backs onto the
siding face** — one proud layer, backs touching, never sinking into the siding plane. Work in one
order: **corner boards → frieze → casings**. Everything here is 1×6 or 1×10 PRIMED from the
separate finish order; fasteners are not on either lumber order, so supply your own nails.

Tick off the parts (cut-list names, see [R01 cut list](r01-cut-list.md)):

| Qty | Part · stock | Cut length | Where |
|---:|:---|:---|:---|
...
### 1. Hang the corner boards

Two boards per corner — one lying flat on each wall face — with their bottom edges on the skirt
top (½″ above the deck). Each board backs on the siding face of its own wall; together they close
the corner. Nail through into the corner framing.

The pairs are not the same length, because the front wall is tall and the back wall is short:
...
from the siding it backs on.](figures/fig-14-a.svg)

### 2. Hang the front and back frieze
...
> ⚠️ **WARNING** — ladder work · the frieze sits about 10 feet up on the front wall · ladder on
> firm ground, move it along rather than reaching

![Fig 14.2 — Front wall from the street, elevation. Stage 11, trim order. Corner boards first,
frieze second, casings third — each piece one proud layer out from the siding.](figures/fig-14-b.svg)
